...
public class Program
{
    ...
    public static int firstNonRepeatingCharacter_method1(String string) // Using built in method
    {
        // O(n) time | O(1) space
    ...
    public static int firstNonRepeatingCharacter_method2(String string) // Using HashMap
    {
        // O(n) time | O(1) space
        // where n is the length of the input string.
        ...
        for (char c : string.toCharArray())
        {
            if (charCountMap.get(c) == 1) return string.indexOf(c);
        }
        return -1;
    }

    public static int firstNonRepeatingCharacter_method3(String string) // Using Array to store count of characters
    {
        // O(n) time | O(1) space
        // where n is the length of the input string.
        // The Constant Space is because the input string has only lowercase
